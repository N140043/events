...
class Events extends Component {
  ...
  onChangeRegistrationStatus = id => {
    eventsList.forEach(each => {
      if (each.id === id) {
        this.setState({
          registrationStatusEvent: each.registrationStatus,
        })
      }
    })
  }
  ...
  render() {
    const {registrationStatusEvent} = this.state
    console.log(registrationStatusEvent)
    return (
      <div className="bg-container">
        ...
        <ActiveEventRegistrationDetails status={registrationStatusEvent} />
      </div>
    )
  }
}
